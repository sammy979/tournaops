...
export async function requireSuperAdmin(_req?: NextRequest): Promise<NextResponse | null> {
  ...
  if (!isSuperAdmin) return NextResponse.json({ error: "Forbidden - admin required" }, { status: 403 })
...
export async function requireOrganizer(_req?: NextRequest): Promise<NextResponse | null> {
  ...
  const allowed = user.role === "ORGANIZER" || user.role === "SUPER_ADMIN" || user.isAdmin === true
  if (!allowed) return NextResponse.json({ error: "Forbidden - organizer required" }, { status: 403 })
...
export async function requirePro(_req?: NextRequest): Promise<NextResponse | null> {
  ...
  if (!isProActive) return NextResponse.json({ error: "Pro required", upgrade: "/dashboard/upgrade" }, { status: 403 })
...
export async function getAuthUser() {
  ...
  return prisma.user.findUnique({
    ...
    select: {
      ...
      role: true, isAdmin: true, isPro: true, proExpiresAt: true,
      organizerName: true, organizerLogo: true,
    },
  })
}