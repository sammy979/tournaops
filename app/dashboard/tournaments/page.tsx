...
export default function TournamentsPage() {
  ...
  const safeStats = (t: Tournament) => {
    try {
      return getTournamentStats(t);
    } catch {
    ...
    <div className="space-y-6">
      ...
      {filtered.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((t) => {
            const stats = safeStats(t);
            return (
              <div
                key={t.id}
                className={`glass-card rounded-2xl p-5 border flex flex-col gap-4 transition-all ${
                  deleting === t.id ? "opacity-50 scale-95" : "border-white/10 hover:border-white/20"
                }`}
              >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { label: "Squads", value: t.teams?.length || 0, icon: Users },
                    { label: "Matches", value: `${stats.completedMatches}/${stats.totalMatches}`, icon: Play },
                    { label: "Rounds", value: t.rounds?.length || 0, icon: Trophy },
                  ].map(s => (
