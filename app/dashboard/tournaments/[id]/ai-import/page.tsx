            ...
            <div className="lg:col-span-3 space-y-4">
              ...
              <div className="flex gap-1 bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">
                {([
                  ...
                  <button key={m.v} onClick={() => { setMode(m.v); setStatus("idle"); setResult(null); setApplied(false); }}
                    className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-colors ${mode === m.v ? "bg-violet-600 text-white" : "text-slate-500 hover:text-slate-300"}`}>
              ...
              {mode === "text" && (
                <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-5">
                  ...
                  <textarea
                    ...
                    rows={8}
                    className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm placeholder-slate-700 focus:outline-none focus:border-violet-500/50 resize-none font-mono leading-relaxed"
                  />
              ...
              {mode === "screenshot" && (
                <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-5">
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-xl p-12 text-center transition-all cursor-pointer ${dragOver ? "border-violet-500/60 bg-violet-500/10" : "border-white/[0.08] hover:border-violet-500/30"}`}
                  >
              ...
              {mode === "generate" && (
                <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-5">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm placeholder-slate-700 focus:outline-none focus:border-violet-500/50 resize-none mb-4"
                  />
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {GENERATE_TEMPLATES.map(t => (
                        <button key={t.label} onClick={() => setText(`Generate a ${t.label} tournament: ${t.desc}`)}
                          className="flex items-start gap-2.5 p-3 bg-white/[0.03] border border-white/[0.06] rounded-xl hover:border-violet-500/30 transition-colors text-left">
              ...
              {status === "idle" && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed text-white py-3 rounded-xl font-semibold transition-colors"
                >
              ...
              {/* Parsing state */}
              {status === "parsing" && (
                <div className="bg-[#0f1117] border border-violet-500/20 rounded-xl p-8 text-center">
                  <div className="w-14 h-14 rounded-full bg-violet-500/15 border border-violet-500/20 flex items-center justify-center mx-auto mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {result.stages.map((s, i) => (
                        <div key={s} className="flex items-center gap-2">
                          <span className="bg-violet-500/15 border border-violet-500/20 text-violet-300 text-xs px-2.5 py-1 rounded-full">{s}</span>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 max-h-40 overflow-y-auto">
                      {result.teams.map(t => (
                        <div key={t.name} className="flex items-center gap-2 bg-white/[0.02] border border-white/[0.04] rounded-lg px-2.5 py-1.5">
                          <div className="w-5 h-5 rounded bg-violet-500/20 flex items-center justify-center text-violet-400 text-xs font-bold flex-shrink-0">{t.tag[0]}</div>
