...
export default function ScoringPresetsPage() {
  ...
  const getPresetPoints = (p: any) => {
    const rule = p.scoringRule || p;
    const placementPoints = rule.placementPoints;
    const killPoints = rule.killPoints ?? p.killPoints ?? 1;
    const wwcdBonus = rule.wwcdBonus ?? p.wwcdBonus ?? 0;
    const firstPlace = Array.isArray(placementPoints)
      ? (placementPoints[0] ?? 0)
      : (placementPoints?.[1] ?? 0);
    return { firstPlace, killPoints, wwcdBonus };
  };
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-yellow-300/70 text-xs">
              Built-in presets are named after popular formats but are approximations.
              Always verify against the latest official tournament rules before competitive use.
            </p>
          </div>
        </div>
      </div>
              ...
              <div key={p.id} className="glass-card rounded-xl p-4 border border-white/10 hover:border-blue-500/30 transition-all">
                ...
                <div className="grid grid-cols-3 gap-2 mb-3 text-xs">
                  <div className="text-center p-2 rounded bg-white/3">
                    <div className="text-yellow-400 font-bold">{pts.firstPlace}</div>
                    <div className="text-gray-600 text-[9px]">1st</div>
                  </div>
                  <div className="text-center p-2 rounded bg-white/3">
                    <div className="text-orange-400 font-bold">{pts.killPoints}</div>
                    <div className="text-gray-600 text-[9px]">Kill</div>
                  </div>
                  <div className="text-center p-2 rounded bg-white/3">
                    <div className="text-purple-400 font-bold">{pts.wwcdBonus}</div>
                    <div className="text-gray-600 text-[9px]">WWCD</div>
                  </div>
                </div>
                <button onClick={() => duplicateFromBuiltIn(p)} className="btn-secondary w-full text-xs py-1.5 flex items-center justify-center gap-1">
      ...
      <div>
        ...
        {presets.custom.length === 0 ? (
          <div className="glass-card rounded-2xl p-8 text-center border-dashed border-2 border-white/10">
            ...
            <button onClick={() => { setEditing(null); setShowBuilder(true); }} className="btn-primary px-5 py-2 text-sm flex items-center gap-1 mx-auto">
            ...
            {presets.custom.map(p => {
              ...
              return (
                <div key={p.id} className="glass-card rounded-xl p-4 border border-white/10 hover:border-blue-500/30 transition-all">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-white font-bold text-sm">{p.name}</h3>
                    {p.isDefault && <Star className="w-3.5 h-3.5 text-yellow-400" />}
                  </div>
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => { setEditing(p); setShowBuilder(true); }} className="flex-1 btn-secondary text-xs py-1.5 flex items-center justify-center gap-1">
