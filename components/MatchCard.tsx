  ...
  return (
    <div className={"rounded-2xl p-3 min-w-[260px] relative border " + (match.isComplete ? "border-green-500/30 bg-green-500/5" : "border-white/10 bg-white/5")}>
      ...
      <div
        ...
        className={"flex items-center justify-between p-3 rounded-xl mb-2 transition-all " + (isWinner1 ? "bg-green-500/20 border border-green-500/40" : editable && match.team1 && match.team2 ? "bg-white/5 hover:bg-purple-500/20 cursor-pointer border border-transparent" : "bg-white/5 border border-transparent")}
      >
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-purple-500/20 border border-purple-500/30 flex items-center justify-center text-xs font-bold text-purple-300">
            {match.team1?.seed ?? "?"}
          </div>
      ...
      <div className="text-center py-1 mb-2">
        <span className="text-[9px] font-bold text-purple-400 tracking-widest">VS</span>
      </div>

      {/* Team 2 */}
      <div
        onClick={() => editable && !editing && setWinner(match.team2)}
        className={"flex items-center justify-between p-3 rounded-xl transition-all " + (isWinner2 ? "bg-green-500/20 border border-green-500/40" : editable && match.team1 && match.team2 ? "bg-white/5 hover:bg-purple-500/20 cursor-pointer border border-transparent" : "bg-white/5 border border-transparent")}
      >
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
        ...
        <div className="mt-3 flex gap-1.5">
          {editing ? (
            ...
            <button onClick={() => setEditing(true)}
              className="w-full py-2 text-xs bg-purple-500/20 text-purple-300 hover:text-white rounded-lg font-bold border border-purple-500/30 transition-all inline-flex items-center justify-center gap-1">
