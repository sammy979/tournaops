  ...
  return (
    <div className={"rounded-2xl p-3 min-w-[260px] relative border " + (match.isComplete ? "border-green-500/30 bg-green-500/5" : "border-white/10 bg-white/5")}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] font-bold text-yellow-300 bg-yellow-500/10 px-2 py-0.5 rounded-full border border-yellow-500/30">
          BO{bestOf}
        </span>
      </div>
      ...
      >
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-yellow-500/20 border border-yellow-500/30 flex items-center justify-center text-xs font-bold text-yellow-300">
            {match.team1?.seed ?? "?"}
          </div>
          <span className={"text-sm font-bold truncate " + (isWinner1 ? "text-white" : "text-yellow-100")}>
        ...
        {editing ? (
          ...
          <div className={"text-2xl font-bold min-w-[32px] text-right " + (isWinner1 ? "text-green-400" : "text-yellow-300/60")}>
            {match.score1 ?? 0}
          </div>
        )}
      </div>
      ...
      >
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          ...
          <span className={"text-sm font-bold truncate " + (isWinner2 ? "text-white" : "text-yellow-100")}>
        ...
        {editing ? (
          ...
          <div className={"text-2xl font-bold min-w-[32px] text-right " + (isWinner2 ? "text-green-400" : "text-yellow-300/60")}>
            {match.score2 ?? 0}
          </div>
        )}
      </div>
        ...
        <div className="mt-3 flex gap-1.5">
          {editing ? (
            ...
            <button onClick={() => setEditing(true)}
              className="w-full py-2 text-xs bg-yellow-500/20 text-yellow-300 hover:text-white rounded-lg font-bold border border-yellow-500/30 transition-all inline-flex items-center justify-center gap-1">
